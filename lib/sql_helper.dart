...
class SQLHelper {
  ...
  Future<int> insertUser(Map<String, dynamic> user) async {
    ...
    // Add the logic to dynamically generate the default user ID
    int defaultUserId = await _generateDefaultUserId();

    // Add the default user ID to the user map
    user[columnUserId] = defaultUserId;

    // Insert the user into the database
    await db.insert(tableUsers, user);

    // Now insert the default product for the new user
    Map<String, dynamic> defaultProduct = {
      columnProductName: 'SkyVoiceAlert 500',
      columnProductDescription: 'LLC’s SkyVoice Alert (FAA Approved) announces the heights through the pilots headset when the LiDAR (Laser) measures a set of predefined altitudes.',
      columnProductAddedDate: '2024-01-01',
      columnProductImagePath: 'assets/dk1.png',
      columnProductBatchName: '2024-01',
      columnProductUserId: defaultUserId, // Use the generated user ID here
    };

    await db.insert(tableProducts, defaultProduct);

    return defaultUserId; // Return the generated user ID if needed
  }

  Future<int> _generateDefaultUserId() async {
    // Add your logic to generate a dynamic default user ID
    // For example, you can query the database to get the maximum user ID and increment it by 1
    Database db = await _instance.database;
    List<Map<String, dynamic>> result = await db.rawQuery('SELECT MAX($columnUserId) as maxId FROM $tableUsers');
